...
${gameData.zombiesKilled > 0 ? `🧟 **${gameData.zombiesKilled} Zombies Purified!**` : ""}

---

**[View Full Game Record on Steem](${steemConfig.getSteemWebUrl()}/@${GAME_RECORDS_CONFIG.account}/${GAME_RECORDS_CONFIG.jsonId}-records)**
...
`;
  ...
  async fetchGlobalGameRecords(maxRecords = 2000, filterUsername = null) {
    const targetAccount = GAME_RECORDS_CONFIG.account;
    const BATCH_SIZE = 100; // Smaller batches for reliability
    const CACHE_KEY = `steemmaze_${targetAccount}_records_cache`; // LocalStorage Key
